...
public:
    void drawRotarySlider(juce::Graphics &g, int x, int y, int width, int height, float sliderPos, float rotaryStartAngle, float rotaryEndAngle, juce::Slider &slider) override{
        ...
        juce::Rectangle<float> dialArea (rx, ry, diameter, diameter);
        g.setColour(juce::Colour::fromFloatRGBA(0.15, 0.15, 0.15, 1)); //center
        g.setGradientFill(juce::ColourGradient::vertical(juce::Colour::fromFloatRGBA(0.392f, 0.584f, 0.929f, 1.0).darker(1.0).darker(0.1), radius, juce::Colour::fromFloatRGBA(0.392f, 0.584f, 0.929f, 1.0).darker(1.0).darker(0.15), diameter));
        g.fillEllipse(dialArea);
